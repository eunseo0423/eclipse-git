...
public class BookSearchMVC extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		textField.setOnAction(e -> {
			// 이벤트 처리
			// Controller 이용해서 Service에 로직 실행 요청을 해야해요.
			// 먼저 controller 객체를 생성할거에요 -> controller class 부터 만들기
			BookSearchByKeywordController controller = new BookSearchByKeywordController();
			
			ObservableList<BookVO> list = 
					controller.getResult(textField.getText()); // 입력상자의 text 가지고 observablelist 가져왕
