...
//			for(long k=0; k<1000000000000000000L; k++); // 천천히 실행중
			...
			try {
				Thread.sleep(2000); // 2초 동안 잔 다음에 일어남
			} catch (InterruptedException e) {
				interrupt(); // 자고 있는 중간에 interrupt를 걸어서 깼지만 초기화됐는걸 다시 interrupt
			} 
...
public class ThreadEx_06_part1 {
	...
	public static void main(String[] args) {
		...
		// 만약 interrupt 걸었는데 이 때 해당 Thread가 하필 이 thread가 sleep(,join,wait 등의) 상태이면
		// exception이 발생 (그래서 try,catch가 있어야함)
